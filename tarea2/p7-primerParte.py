"""
Problema 7: Ataque de fuerza bruta a DES usando lista de claves (words.txt)
Se reutiliza la implementación manual de DES-ECB del Problema 6.
"""

import base64

# Copiamos exactamente el problema 6
...
# Ataque fuerza bruta WORDS.TXT


def ataque_fuerza_bruta(archivo_claves, texto_cifrado_b64):
    ...
    for clave in claves:
        clave_bytes = clave.encode('utf-8')
        if len(clave_bytes) != 8:
            clave_bytes = clave_bytes.ljust(8, b'\x00')[:8]
        try:
            texto = descifrar_des_ecb_base64(texto_cifrado_b64, clave_bytes.decode('utf-8'))
            if all(c.isprintable() for c in texto):
...
if __name__ == "__main__":
    cifrado = "h+F7XMoHpF0="
    clave, descifrado = ataque_fuerza_bruta("words.txt", cifrado)

"""
     RESPUESTA A LA PREGUNTA OBLIGATORIA:
    Si no se contara con la lista reducida de claves, sería necesario un ataque
    de fuerza bruta sobre el espacio completo de 2^56 claves DES.
    Una computadora básica probando 10 millones de claves/segundo tardaría
    tiempo_seg = (2**56) / 10_000_000
    tiempo_anios = tiempo_seg / (60*60*24*365)
    Lo que equivale a aproximadamente 228 años.
    Esto demuestra la importancia de las buenas prácticas de seguridad y la
    elección de claves robustas no basadas en diccionarios.
